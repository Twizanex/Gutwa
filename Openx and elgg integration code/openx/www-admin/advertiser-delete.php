...
$spec_id = $_GET['spec_id'];
    ...
    while (list(,$spec_id) = each($ids)) {
       ...
       // OA_Permission::enforceAccessToObject('clients', $clientid, false, OA_Permission::OPERATION_DELETE);
        ...
        $doBanner_spec->spec_id = $spec_id;
        if ($doBanner_spec->get("spec_id",$spec_id)) {
            $aBanner = $doBanner_spec->toArray();
        }

        $doBanner_spec->delete();
    }
    ...
    if (count($ids) == 1) {
        $translated_message = $translation->translate ('Banner specification has been deleted', array(
            htmlspecialchars($aBanner['spec_name'])
        ));
    } else {
        $translated_message = $translation->translate ('Banner specification have been deleted');
    }
...
if(basename($_SERVER['HTTP_REFERER'] ) =='banner-specification-index.php') {

$return_url = 'banner-specification-index.php';
