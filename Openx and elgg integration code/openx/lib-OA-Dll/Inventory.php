...
require_once MAX_PATH . '/lib/OA/Dll/InventoryInfo.php';
...
class OA_Dll_Inventory extends OA_Dll
{
    /**
	 ...
	 function _setInventoryDataFromArray(&$oInventory, $InventoryData)
    {
        ...
        $InventoryData['detail']  = $InventoryData['detail'];
        $InventoryData['conversion'] = $InventoryData['conversion'];
	 $InventoryData['clicks']  = $InventoryData['clicks'];
        $InventoryData['impressions'] = $InventoryData['impressions'];
 $InventoryData['cost'] = $InventoryData['cost'];
	...
	function getInventoryList(&$aInventoryList)
    {
        ...
        $doInventory = OA_Dal::factoryDO('inventory');
        $doInventory->find();

        while ($doInventory->fetch()) {
            $InventoryData = $doInventory->toArray();

            $oInventory = new OA_Dll_InventoryInfo;
