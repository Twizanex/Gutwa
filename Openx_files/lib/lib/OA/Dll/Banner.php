...
class OA_Dll_Banner extends OA_Dll
{
	...
	$bannerData['denial_message']	= $bannerData['denial_message'];
	$bannerData['keyword']          = $bannerData['keyword'];
	$bannerData['statustext']          = $bannerData['statustext'];

        $oBanner->readDataFromArray($bannerData);
        return  true;
    }

    function _validateImage($filename,$content, &$oImage)
    {
        if (empty($filename)) {
	...
	$file_content = file_get_contents($content);
        $oImage = OA_Creative_File::factoryString($filename, $file_content);
...
//$oBanner['aImage'] = array('filgename'=>$oBanner['prepend'],'cogntent' =>$oBanner['append']);
//print_r($oBanner);
            if (isset($oBanner->append)) {
                if (!$this->_validateImage($oBanner->prepend,$oBanner->append, $this->oImage)) {
                    return false;
                }
...
//die("test");
	...
	//$bannerData['keyword']          = 
	//$bannerData['statustext']          = $oBanner->statustext;
...
$bannerData['keyword']          = $oBanner->Keyword;
	$bannerData['statustext']          = $oBanner->statusText;
